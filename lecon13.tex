...
\begin{itemize}
    ...
    \item \ac{وَعَلَيْكُمُ السَّلَامُ}{也祝您平安（ \arm{عَلَيكُمْ} 词尾 \arm{ـمْ} 遇后 \arm{ـْ} 需变动符，读作 \arm{ـمُ سْـ}）}
    \item \ac{سَلَامَةٌ}{和平，平安}
    \item \ac{مَعَ السَّلَامَةِ}{再见（习惯上只用阴性）}
    \item \ac{حَمْدٌ}{赞颂}
    \item \ac{اَللّٰهُ}{伊斯兰教徒等群体所信奉的唯一主宰}
    \item \ac{لِلّٰهِ}{属于 \arm{ﷲ}}
    \item \ac{اَلْحَمْدُ لِلّٰهِ}{赞美 \arm{ﷲ}}
...
\begin{attention}
    \arm{اَللّٰهُ} 中带了一个冠词。此外，该词亦有独立编码 \arm{ﷲ} 。 \arm{ـلّـ} 上小竖表示发开口长音 \arm{ـلَّاـ} 。此处发音应浑厚。 
    ...
    \arm{مَخَادُّ} 中长音接静符号变短音，读作 \arm{مَخَ دُّ} 。
\end{attention}
...
\begin{note}
    \begin{center}
        \input{img/img13.1.tex}
        \input{img/img13.2.tex}
    \end{center}
\end{note}
...
\begin{attention}
    ...
    \arm{زَمِيلِي الْقَدِيمُ} 一例中，使用属格接尾代名词将 \arm{زَمِيلٌ} 变成了确指名词。
\end{attention}
...
\red{树} 是起语，\blue{房前} 是述语。倒装名词句中，起语称作``\emph{退后起语}''，通常用泛指名词（注意区别上例中 \arm{اَلشَّجَرَةُ} 变为 \arm{شَجَرَةٌ}）；述语称作``\emph{前置述语}''，通常为介词短语或方位词组。

若要表示否定，在句首加\emph{残缺动词} \arm{لَيْسَ م لَيْسَتْ} ，配合退后起语，注意阴性形式词尾标静符。

以下为一些例子：

\begin{itemize}
    \item \ac{لِيُمْنَى قَلَمٌ.}{\arm{يُمْنَى} 有一支笔。}
    \item \ac{القلمُ لِيُمْنَى.}{笔属于 \arm{يُمْنَى}。}
    \item \ac{لَيْسَ لِيُمْنَى قَلَمٌ.}{\arm{يُمْنَى} 没有笔（$\nexists$笔）。}
    \item \ac{القلمُ لَيْسَ لِيُمْنَى. = لَيْسَ القلمُ لِيُمْنَى.}{笔不属于 \arm{يُمْنَى}（$\exists$笔）。}
\end{itemize}

\begin{note}
    对于否定情况，既然前面加的玩意叫残缺动词，合理怀疑它就不再是名词句了，但课上没明确说。配合前课程说过，动词句的话一般是开头动词，不太知道 \arm{القلمُ لَيْسَ لِيُمْنَى.} 这种说法是什么个结构。不过，课上明确说了两种语序都行。
\end{note}
...
后者正装句为 \arm{السَّاعَةُ وَالْخَرِيطَةُ (لَيْسَتْ) عَلَى الْجِدَارِ.} ，注意谁配合谁。